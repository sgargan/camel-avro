...
public class AvroNettyTransportTest extends AbstractAvroTransportTest {

    @Override
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() {
                from("avro:netty://0.0.0.0:4567/?protocol=orderingService").to("mock:order");
            }
        };
    }
    ...
    public Transceiver createTransceiver(int port) throws Exception {
        return new NettyTransceiver(new InetSocketAddress("0.0.0.0", port));
    }

    public Server createServer(int port, MockOrderingService mock) throws IOException {
        return new NettyServer(new SpecificResponder(OrderProcessingService.PROTOCOL, mock), new InetSocketAddress("0.0.0.0", port));
    }
